    ...
    while (!q.empty() && result.totalNodes < options.maxResults) {
        ...
        for (const auto& child : sortedChildren) {
            ...
            if (nodeIt->second.type == SpaceNodeType::ROOM) {
                nodes_.at(nodeId].childCount++;
                nodes_.at(nodeId].totalDescendantCount++;
            }
